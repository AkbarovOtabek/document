...
export default {
  ...
  emits: ["openInfo", "openAdd"],
  data() {
    return { hovering: false };
  },
  computed: {
    size() {
      return this.side === "center" ? 240
        : this.side === "management" ? 180
        : this.side === "department" ? 150 : 115;
    },
    // Автомасштаб надписей (по длине заголовка и ФИО)
    fsTop() {
    ...
    fioLines() {
      ...
      // Разбиваем ФИО на 2–3 строки (мягкий wrap ~22 символов)
...
.avatar-figure{ width:44%; aspect-ratio:1/1; }
.avatar-figure svg{ width:100%; height:100%; fill:#000; }

.label{
  position:absolute; left:50%; transform:translateX(-50%);
  color:#111; text-align:center; font-weight:500;
  word-break: break-word;
  overflow: hidden;
}
.label.top{
  top:10%; width:78%;
  /* font-size: var(--fs-top, 16px); */
  font-size:9px;
  line-height:1.15;
...
.sidebtn{
  ...
  border:none; background:#111; color:#fff; font-weight:800;
  display:grid; place-items:center; cursor:pointer;
  box-shadow:0 6px 16px rgba(0,0,0,.25);
  transition: opacity .2s ease, transform .2s ease;
  opacity:.96;
}
.sidebtn.left{ left:-24px; }
.sidebtn.right{ right:-24px; }
</style>
