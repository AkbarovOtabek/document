    ...
    <section class="cards">
      ...
      >
        ...
        >
          ...
          <div class="grid2">
            <div>
              <strong>Адрес:</strong> <span class="muted">{{ o.address || "—" }}</span>
            </div>
            <div>
              <strong>Lotus:</strong> <span class="muted">{{ o.lotus || "—" }}</span>
            </div>
            <div>
              <strong>Тел.:</strong> <span class="muted">{{ o.phone || "—" }}</span>
            </div>
            <div>
              <strong>Email:</strong> <span class="muted">{{ o.email || "—" }}</span>
            </div>
          </div>
          ...
          <div class="card-actions">
            ...
            <button
              class="btn subtle"
              @click.stop="$router.push({ name: 'org-detail', params: { slug: o.slug } })"
            >
              Подробнее
            </button>
          </div>
        ...
        <div class="modal-card" @mousemove="onTilt" @mouseleave="resetTilt">
          <div class="modal-inner card-inner">
            ...
            <div class="grid2 form-grid">
              <div class="row">
                <label>Адрес</label><input v-model="createForm.address" class="input" type="text" />
              </div>
              <div class="row">
                <label>Lotus</label><input v-model="createForm.lotus" class="input" type="text" />
              </div>
              <div class="row">
                <label>Телефон</label><input v-model="createForm.phone" class="input" type="text" />
              </div>
              <div class="row">
                <label>Email</label><input v-model="createForm.email" class="input" type="email" />
              </div>
            </div>

            <div class="row">
              <label>Логотип (иконка банка)</label>
      ...
      <div class="modal" v-if="showEdit" @click.self="showEdit = false">
        <div class="modal-card" @mousemove="onTilt" @mouseleave="resetTilt">
          <div class="modal-inner card-inner">
            ...
            <div class="grid2 form-grid">
              <div class="row">
                <label>Адрес</label><input v-model="editForm.address" class="input" type="text" />
              </div>
              <div class="row">
                <label>Lotus</label><input v-model="editForm.lotus" class="input" type="text" />
              </div>
              <div class="row">
                <label>Телефон</label><input v-model="editForm.phone" class="input" type="text" />
              </div>
              <div class="row">
                <label>Email</label><input v-model="editForm.email" class="input" type="email" />
              </div>
            </div>
            ...
            <div class="row">
              ...
              <label class="check"
                ><input type="checkbox" v-model="editForm.keepLogo" /> оставить текущий</label
              >
            </div>
...
.chart .line {
  ...
  stroke: var(--accent);
  stroke-width: 2.4;
  filter: drop-shadow(0 2px 6px rgba(43, 223, 131, 0.35));
}
