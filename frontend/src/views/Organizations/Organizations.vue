  ...
  <div class="scene" :class="{ dark: isDark }">
    ...
    <section class="hero">
      <div class="hero-left">
        ...
        <p class="lead">
          Управляй банками, платёжными организациями и системами через удобный интерфейс. Гибкие
          фильтры, мгновенный поиск, чистый API.
        </p>

        <div class="cta-row">
          <button class="btn primary big" @click="showCreate = true">Создать категорию</button>
          <button class="btn ghost big" @click="fetchCategories">Обновить список</button>
    ...
    <section class="cards">
      ...
      >
        <div class="card-inner">
          ...
          <div class="metrics">
            <div>
              <strong>{{ c.objects_count ?? 0 }} </strong><span> объектов</span>
            </div>
            <div>
              <strong>{{ c.today_count ?? 0 }} </strong><span> за сегодня</span>
            </div>
          </div>
          ...
          <div class="modal-inner card-inner">
            ...
            <div class="row">
              ...
              <input
                ...
                class="input"
                type="text"
                placeholder="Платежи / Банки / Системы"
              />
            </div>
            <div class="row">
      ...
      <div class="modal" v-if="showEdit" @click.self="closeEdit">
        <div class="modal-card" @mousemove="onTilt" @mouseleave="resetTilt">
          <div class="modal-inner card-inner">
            ...
            <div class="row submit">
              <button class="btn primary" :disabled="saving" @click="updateCategory">
                Сохранить
              </button>
...
.scene {
  ...
  --primary: #19c46d;
  --primary-ink: #0ea95b;
  --ring: rgba(25, 196, 109, 0.25);
  --line: #e6e8ee;
  min-height: calc(100vh - 64px);
  background: radial-gradient(1200px 600px at 70% -100px, rgba(25, 196, 109, 0.18), transparent 60%),
    var(--bg);
  color: var(--ink);
  display: flex;
...
.scene.dark {
  ...
  --primary: #2bdf83;
  --primary-ink: #17b469;
  --ring: rgba(43, 223, 131, 0.22);
  --line: #222a36;
  background: radial-gradient(1300px 700px at 70% -120px, rgba(43, 223, 131, 0.14), transparent 60%),
    var(--bg);
}
...
.hl {
  color: var(--primary);
  text-shadow: 0 0 24px rgba(25, 196, 109, 0.35);
}
.lead {
  color: var(--muted);
  margin: 0 0 18px;
...
.cube {
  ...
  animation: idle 8s ease-in-out infinite alternate;
}
@keyframes idle {
  0% {
...
.face {
  ...
  background: linear-gradient(145deg, rgba(255, 255, 255, 0.09), rgba(255, 255, 255, 0.03)),
    linear-gradient(145deg, #2bdc83, #15a85a);
...
.halo {
  ...
  height: 26px;
  filter: blur(8px);
  background: radial-gradient(closest-side, rgba(25, 196, 109, 0.55), transparent 70%);
}
