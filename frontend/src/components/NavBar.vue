  ...
  data() {
    return {
      items: [
        ...
        { to: '/search-documents', icon: 'chatbubble-ellipses-outline', label: 'Поиск писем',      match: ['/search-documents'] },
        { to: '/statistika',       icon: 'analytics-outline',           label: 'Статистика',       match: ['/statistika'] },
...
.navigation ul{ display:flex; width: 550px; position:relative; }
.navigation ul li{ position: relative; width: 130px; height: 70px; z-index: 1; }
.navigation ul li a{
...
.navigation ul li a .icon{ line-height: 75px; font-size: 1.5em; transition: .5s; color: var(--clr); }
...
.navigation ul li a .text{
  ...
  transition:.5s; opacity:0; transform: translateY(20px);
}
...
.indicator{
  ...
  width:70px; height:70px; left:20px;
  background: var(--button-green-dark);
  border-radius:50%; border:7px solid var(--clr);
  transition: transform .35s cubic-bezier(.22,.61,.36,1);
}
.indicator::before,
.indicator::after{
  content:""; position:absolute; top:50%; width:20px; height:20px; background:transparent;
}
.indicator::before{ left:-22.5px; border-top-right-radius:20px; box-shadow: 0 -10px 0 0 var(--clr); }
.indicator::after{ right:-22.5px; border-top-left-radius:20px; box-shadow: -1px -10px 0 0 var(--clr); }

/* тёмная тема */
.navigation.dark{ background: var(--clr); }
.navigation.dark ul li a .icon,
.navigation.dark ul li a .text{ color: #fff; }
.navigation.dark .indicator{ border:7px solid #fff; }
.navigation.dark .indicator::before,
.navigation.dark .indicator::after{ box-shadow: 0 -10px 0 0 #fff; }
</style>
